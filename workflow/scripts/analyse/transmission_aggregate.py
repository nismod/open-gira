...
transmission_paths, storm_tot, years_tot = find_storm_files(
    "edges", output_dir, region_eval, sample_eval, nh_eval, thrval
)
...
storms_increasing_severity = stats.sort_values("reconstruction cost", ascending=True)[
    "Storm ID"
]
storm_id_metrics_weighting = dict(
    zip(storms_increasing_severity, return_periods)
)  # dictionary {storm1: return_period_of_storm1, storm2: ... }
...
if len(transmission_paths) == 0:
    ...
    countries_relevant = (
        set()
    )  # list of countries which contain relevant data (rest can be ignored)
    transmission_dict = dict()
    transmission_lst = []
    for ii, transmission_path in tqdm(
        enumerate(transmission_paths),
        desc="Iterating transmission lines",
        total=len(transmission_paths),
    ):

        storm = os.path.basename(transmission_path).split("_n")[-1][
            :-5
        ]  # extract storm

        transmission = gpd.read_file(transmission_path)[
            ["link", "geometry", "box_id", "reconstruction_cost"]
        ]
        countries_relevant = countries_relevant.union(
            set().union(
                *[set(boxes_country[box]) for box in transmission.box_id.unique()]
            )
        )  # update relevant countires
        ...
        weighting_factor = (
            1 / years_tot
        )  # return period weighting factor for annual expected. Note this is a pretty good approximation to the trapezium rule for large n
        new_dict = {
            transmission_indiv.link: [
                1,
                transmission_indiv.geometry,
                transmission_indiv.reconstruction_cost,
                weighting_factor * transmission_indiv.reconstruction_cost,
            ]
            for transmission_indiv in transmission.itertuples()
            if transmission_indiv.link not in transmission_dict.keys()
        }
    ...
    transmission_master = gpd.GeoDataFrame(
        {
            "link": transmission_dict.keys(),
            "count_damage": [x[0] for x in transmission_dict.values()],
            "geometry": [x[1] for x in transmission_dict.values()],
            "reconstruction_cost": [x[2] for x in transmission_dict.values()],
            "reconstruction_cost_annual_expected": [
                x[3] for x in transmission_dict.values()
            ],
        }
    )
    ...
    transmission_master["reconstruction_cost_annual_expected_per_km"] = [
        transmission_master["reconstruction_cost_annual_expected"].iloc[jj]
        / eval_dist_lst_output[jj]
        if eval_dist_lst_output[jj] != 0
        else 0
        for jj in range(len(transmission_master))
    ]

    # Fix geometries cut by units
    links = set(transmission_master.link.unique())
    link_boxes = set(
        [f"box_{l.split('__')[0].split('_')[-1]}" for l in links]
        + [f"box_{l.split('__')[1].split('_')[-1]}" for l in links]
    )
    for ii, box_id in enumerate(link_boxes):
        if ii == 0:
            box_network = gpd.read_file(
                os.path.join(
                    output_dir,
                    "power_processed",
                    "all_boxes",
    ...
    transmission_master["geometry"] = transmission_master["link"].map(
        geom_dict
    )  # fix truncated geoms
    ...
    code_geoms_gpd["len"] = [
        len(g.geoms) for g in code_geoms_gpd["geometry"]
    ]  # include number of polygons (higher the more computationally expensive intersect function is
    ...
    transmission_master = transmission_master[
        ~transmission_master.geometry.isna()
    ]  # remomve null geometries
    post_len = len(transmission_master)
    if post_len / pre_len < 0.7 and pre_len > 50:  # small check
        print(
            "Possibly issue as 30%+ of transmission lines are being removed for having no geometry"
        )
    ...
    for geom_area in tqdm(
        ...
        s1 = time.time()
        transmission_master_bounded = transmission_master[
            (transmission_master.lon_min >= minx)
            & (transmission_master.lon_max <= maxx)
            & (transmission_master.lat_min >= miny)
            & (transmission_master.lat_max <= maxy)
        ]  # only check within bounding box

        if len(transmission_master_bounded) >= 1:

            bool_list = [
                True if t.intersects(geom_area.geometry) else False
                for t in transmission_master_bounded.geometry
            ]
            ...
            if len(overlap_transmission) >= 1:
                ...
                map_dict_ll[geom_area.code] = eval_dist(
                    overlap_transmission
                )  # note down degree length

                if (
                    geom_area.code not in map_dict.keys()
                    and "reconstruction_cost" in overlap_transmission.columns.values
                ):
                    map_dict[geom_area.code] = sum(
                        overlap_transmission.reconstruction_cost
                        * overlap_transmission.count_damage
                    )
                    map_dict_ae[geom_area.code] = sum(
                        overlap_transmission.reconstruction_cost_annual_expected
                        * overlap_transmission.count_damage
                    )
                else:
                    map_dict[geom_area.code] = map_dict[geom_area.code] + sum(
                        overlap_transmission.reconstruction_cost
                        * overlap_transmission.count_damage
                    )
                    map_dict_ae[geom_area.code] = map_dict_ae[geom_area.code] + sum(
                        overlap_transmission.reconstruction_cost_annual_expected
                        * overlap_transmission.count_damage
                    )

                transmission_master = transmission_master[
                    ~transmission_master.link.isin(overlap_transmission.link)
                ]  # remove from code_geoms_gpd_transmission_master

    code_geoms_gpd["reconstruction_cost_sum"] = (
        code_geoms_gpd["code"].map(map_dict).fillna(0)
    )
    code_geoms_gpd["reconstruction_cost_avg"] = (
        code_geoms_gpd["reconstruction_cost_sum"] / storm_tot
    )  # average over all hitting storms
    code_geoms_gpd["reconstruction_cost_annual_expected"] = (
        code_geoms_gpd["code"].map(map_dict_ae).fillna(0)
    )
    code_geoms_gpd["reconstruction_cost_annual_expected_fraction_normalised"] = (
        code_geoms_gpd["reconstruction_cost_annual_expected"]
        / (code_geoms_gpd["code"].map(map_dict_ll))
    ).fillna(
        0
    )  # This metric divides by the total unit length of transmission lines in that region.
