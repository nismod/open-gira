...
metric_file = {
    "EAD": "empirical_reconstruction cost_plotting_data.csv",
    "EACA": "empirical_effective population affected_plotting_data.csv",
}


for metric in metrics:
    write_lst = []
    data_constant = pd.read_csv(
        os.path.join(
            output_dir,
    ...
    for ii, model in enumerate(models_future):
        ...
        if ii == 0:
            ...
            data = pd.merge(
                data, data_new, how="outer", on=["x"], suffixes=(f"_A{ii}", f"_B{ii}")
            )
