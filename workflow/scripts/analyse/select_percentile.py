...
for metric in metrics_target:
    ...
    storm_select = stats.sort_values(
        metric_sortby[metric], ascending=False
    )  # select percentile

    if (
        idx <= len(stats) - 1
    ):  # if idx > number of storms, there is no damage for that storm
        ...
        storm_id_directory = os.path.join(
            output_dir,
            "power_intersection",
            "storm_data",
            "individual_storms",
