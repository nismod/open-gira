...
examine_file = os.path.join(folder_agg, f"targets_geo_top{top_select}{increased_severity_sort_bool}percent.gpkg")
...
cg_maxx_bool = cg_bounds['maxx'] < qf_minx  # True if max region x is less than min quantile x
cg_minx_bool = cg_bounds['minx'] > qf_maxx  # True if min region x is more than max quantile x
cg_maxy_bool = cg_bounds['maxy'] < qf_miny  # True if max region is less than min quantile y
cg_miny_bool = cg_bounds['miny'] > qf_maxy  # True if min region is more than max quantile y
bool_keep = [False if cg_maxx_bool[x] + cg_minx_bool[x] + cg_maxy_bool[x] + cg_miny_bool[x] >= 1 else True for x in range(len(cg_bounds))]  # if 1 or higher, then there is at least one True element
...
for geom_area in tqdm(code_geoms_gpd.itertuples(), total=len(code_geoms_gpd), desc='geom_intersect'):
    ...
    if len(overlap_quantile) >= 1:
        ...
        if geom_area.code not in map_dict.keys():
            map_dict[geom_area.code] = {metric: [] for metric in metric_keys}  #dict(zip(metric_keys, [[]]*len(metric_keys)))
