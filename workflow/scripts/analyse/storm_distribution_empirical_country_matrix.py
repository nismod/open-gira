...
def plot_relation_matrix(matrix, title, fig_num):
    ...
    plt.xticks(list(country_index.values()), labels=list(country_index.keys()), rotation=90)
...
for country_pair, count in countries_overlap_master.items():
    ...
    tot_storms = country_storm_count[country1] + country_storm_count[country2] - count # total number of storms for both countries (union)
    idx1 = max(country_index[country1], country_index[country2])
    idx2 = min(country_index[country1], country_index[country2])
    country_matrix_unint[idx1, idx2] = count/tot_storms  # add the count (intersection) for the countries using the country_index index for the respecive countries as a fraction: intersction/union

    country_matrix_condprob[country_index[country1], country_index[country2]] = count/country_storm_count[country1]  # given country 1 is hit, what is likelyhood of country 2 being hit
