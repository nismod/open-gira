...
if __name__ == "__main__":
    ...
    threshold = (
        opts_dict["exposure_threshold"]
        if "exposure_threshold" in opts_dict.keys()
        else 0.5
    )
    scaling_factor = (
        opts_dict["scaling_factor"] if "scaling_factor" in opts_dict.keys() else 0.1
    )
    resampling_mode = (
        opts_dict["resampling_mode"]
        if "resampling_mode" in opts_dict.keys()
        else "bilinear"
    )
    ...
    with rasterio.open(hazard) as h_file:
        ...
        with rasterio.open(tmp_file_path) as tmp_file:
            ...
            transform = tmp_file.transform * tmp_file.transform.scale(
                (tmp_file.width / scale_width), (tmp_file.height / scale_height)
            )
            # Resampling
            with rasterio.open(
                f"{output_path}",
                ...
                data = tmp_file.read(
                    out_shape=(tmp_file.count, scale_height, scale_width),
