...
if __name__ == "__main__":
    ...
    if len(nh_toprocess) != 0:
        ...
        s = time.time()
        output = []
        for ii, grid_box_indiv in enumerate(grid_box.itertuples()):
            #print(ii)
            output += [TC_analysis(
                grid_box_indiv.latitude,
                grid_box_indiv.longitude,
                grid_box_indiv.ID_point,
                grid_box_indiv.box_id,
                grid_box_indiv.region,
                sample,
                nh_toprocess,
                ii,
                len(grid_box),
