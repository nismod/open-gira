...
print("Loading wind")
windfile = os.path.join("data", "intersection", "storm_data", "all_winds", f'TC_r{region}_s{sample}_n{nh}.csv')
if not os.path.isfile(windfile):
    raise OSError(f"Wind file should exist but doesnt (TC_r{region}_s{sample}_n{nh}.csv)")
...
routeid_damaged = {}  # marks which source_sink routes already damaged -> do not double count. Form: {source:[target,target,...], source:[target, target,...],...}
...
for jj, box_id in enumerate(box_id_affected):
    ...
    for ii, edge in tqdm(enumerate(edges_to_check), desc=f'storm_{nh}-{box_id}: affected edges', total=len(box_edges_affected)):
        s1 = time.time()

        # preprocess the edge
        edge_dict = sorted_gdp[edge]  # returns a dictionary of all source_sink paths running through that edge
        edge_dict = {k.replace('source_','s').replace('_box_','b').replace('target_','t'): v for k, v in edge_dict.items()}
        ...
        for sourcesink, v in edge_dict.items():
            ...
            if source in routeid_damaged.keys():  # key exists
                if sourcesink.split('_')[1] in routeid_damaged[source]:  # sourcesink already counted
                    continue  # do not add to dmg
            dmg += v
        #print('time for t1 ', time.time()-t1)

        # find target operations
        if operationfind:  # if the operation value of the target is desired  # TODO note this only takes into account targets in the affected boxes
            t_gdp =  [[route_id_.split('_')[1], v] for route_id_, v in edge_dict.items() if route_id_.split('_')[1] not in routeid_damaged.get(route_id_.split('_')[0], [])]  # list of lists containing [targetnumber, gdp] only if not already damaged
            t_dam = list(set([t[0] for t in t_gdp]))  # make unique
            for t_indiv in t_dam:  # go through unique (undamaged) targets
                totdamage_t = sum([t[1] for t in t_gdp if t[0] == t_indiv])  # sum damage for each (unique, undamaged) target
        ...
        t2 = time.time()
        sources_new = [source for source in sources if source not in routeid_damaged.keys()]
        routeid_damaged.update(dict(zip(sources_new, [[]]*len(sources_new))))  # add new keys
...
if len(edges_affected) != 0:  # to prevent writing empty dataframe
    edges_affected.to_file(os.path.join(storm_path, f"edges_affected__storm_r{region}_s{sample}_n{nh}.gpkg"), driver='GPKG')
if len(polys_affected) != 0:  # to prevent writing empty dataframe
    polys_affected.to_file(os.path.join(storm_path, f"region_affected__storm_r{region}_s{sample}_n{nh}.gpkg"), driver='GPKG')


# if other boxes (in which no direct damaged units) contain targets with a reduced operation then add to targets file
if operationfind:
    box_id_unaccounted = list(set([key[key.find('box'):] for key in targetsdamaged.keys() if key[key.find('box'):] not in box_id_affected]))  # get set of boxes not in box_id_affected but have target damage
    for box_id_unaccounted_indiv in box_id_unaccounted:  # for each unaccounted for box, add to targets gpkg
        targets_unaccounted_indiv = gpd.read_file(os.path.join("data","processed", "all_boxes", box_id_unaccounted_indiv, f"targets_{box_id_unaccounted_indiv}.gpkg"))
...
targets.to_file(os.path.join(storm_path, f"targets__storm_r{region}_s{sample}_n{nh}.gpkg"), driver='GPKG')
...
if len(TC) != 0:
    ...
    storm_track.to_file(os.path.join(storm_path, f"storm_track_r{region}_s{sample}_n{nh}.gpkg"), driver='GPKG')
...
damagescsvpath = os.path.join("data", "intersection","storm_data", "individual_storms", f"storm_{nh}", f"storm_r{region}_s{sample}_n{nh}.txt")
