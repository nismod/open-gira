...
class WayParser(osmium.SimpleHandler):
    ...
    def find_shared_nodes(self, file_path: str, locations: bool = False, **kwargs) -> dict[int, int]:
        """
...
class NodeParser(osmium.SimpleHandler):
    ...
    def node(self, node: osmium.osm.Node) -> None:
        ...
        for tag_name in self.tags_to_preserve:
            base_input[f"tag_{tag_name}"] = node.tags[tag_name] if tag_name in node.tags else None
...
class WaySlicer(osmium.SimpleHandler):
    ...
    def way(self, way: osmium.osm.Way) -> None:
        ...
        base_input = {}
        for tag_name in self.tags_to_preserve:
            base_input[f"tag_{tag_name}"] = way.tags[tag_name] if tag_name in way.tags else None

        node_locations: tuple[float, float] = []
        shared_nodes_used: list[dict] = []
        # create parallel data structure to enable lookup by location
        nodes_indexed_by_location: defaultdict[float, dict[float, osmium.osm.NodeRef]] = defaultdict(dict)
        ...
        if not isinstance(way_linestring_cut_to_bbox, (LineString, MultiLineString)):
...
if __name__ == "__main__":
    ...
    bbox = shapely_box(box.bottom_left.lon, box.bottom_left.lat, box.top_right.lon, box.top_right.lat)
    ...
    way_slicer = WaySlicer(shared_nodes=shared_nodes, tags_to_preserve=keep_tags, bounding_box=bbox)
