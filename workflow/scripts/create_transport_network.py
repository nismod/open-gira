...
def clean_OSM_ways(ways: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    if not necessary_columns.issubset(set(ways.columns.values)):
        raise ValueError(
            f"{ways.columns.values} does not contain {set(ways.columns.values) - necessary_columns}"
        )
    ...
    for column_name, dtype, nullable in type_conversion_data:
        ways[column_name] = ways[column_name].apply(
...
if __name__ == "__main__":
    try:
        ...
        network_type = snakemake.config[
            "transport_type"
        ]  # used to label edge IDs and index transport tariffs
    ...
    network = create_network(
        edges=clean_OSM_ways(osm_extract), nodes=None, node_edge_prefix=network_type
    )
