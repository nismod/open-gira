...
def combine_networks(box_id_orig):  # NOTE THAT THIS WILL HAVE TO RE-WORK OUT ALL SURROUNDING BOXES
    ...
    fname = os.path.join("data","processed","all_boxes", box_id_orig, f"network_{box_id_orig}.gpkg")
    ...
    while len(to_test) >= 1:
        ...
        for test_box in test_boxes_copy:
            if f"{min(base_box, test_box)}_{max(base_box, test_box)}" in checked:  # ensures no re-checks
                continue
            checked.append(f"{min(base_box, test_box)}_{max(base_box, test_box)}")
            try:  # try and open the network file
                with open(os.path.join("data","processed","all_boxes", f"box_{test_box}", f"connector_box_{test_box}.txt"), 'r') as file_ex:
            ...
            test_fname = os.path.join("data","processed","all_boxes", f"box_{test_box}", f"network_box_{test_box}.gpkg")
            try:
            ...
            for conn in test_box_base_connections:  # for each box connection to base_box
            ...
            if len(test_box_base_connections_keep) >= 1:
                ...
                all_connections_lst = [[connection_det[i] for connection_det in test_box_base_connections_keep] for i in range(len(test_box_base_connections_keep[0]))]  # reorganise connection details
                all_connections_lst[-2] = [sw.loads(str_ls) for str_ls in all_connections_lst[-2]]  # convert to geom
                ...
                to_test.append(test_box) # since test_box is connected, note to test all around test_box too
...
target_sources_df.to_csv(os.path.join("data","processed", "all_boxes", box_id, f"target_source_allocation_{box_id}.csv"))
fname_targets = os.path.join("data","processed", "all_boxes", box_id, f"targets_{box_id}.gpkg")
...
targets.to_file(os.path.join("data","processed", "all_boxes", box_id, f"targets_with_allocation_{box_id}.gpkg"), driver='GPKG')
