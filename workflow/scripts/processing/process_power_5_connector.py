...
network_paths = glob.glob(os.path.join("data", 'processed', 'all_boxes',"box_*", "network_box_*.gpkg"))  # finds the network_{box_id}.gpkg files that exist in all_boxes

for network_path in tqdm(network_paths, desc='connecting all boxes', total=len(network_paths)):
    s = time.time()
    # extract box numbers
        ...
        for id_ex in examine:
            ...
            if os.path.exists(path_test):
                ...
                if list(gdf_ex.id) != [None]:  # check not dummy
                    ...
                    update_lst = [[f'edge_X_{from_box}__{to_box}',from_source_id,f'cb_{from_box}__{to_box}','transmission',from_idx,to_ex,str(LineString([fromto_geom, fromto_geom])),f"{from_idx}__{to_ex}"] for from_idx, to_ex, from_box, to_box, from_source_id, fromto_geom in zip(duplicates.id_idx, duplicates.id_ex, duplicates.box_id_idx, duplicates.box_id_ex, duplicates.source_id_idx, duplicates.geometry)]
                    #update_dict = {i: j for i, j in zip(duplicates.id_idx, duplicates.id_ex)}
                    #portal_lst_ex+=update_lst

                    portal_dict.update({f"box_{id_ex}":update_lst})  # update master dict with neighbour dict
