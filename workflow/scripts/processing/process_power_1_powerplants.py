...
if __name__ == '__main__':
    ...
    powerplants = pd.read_csv(powerplant_file, dtype={'other_fuel3':object})  # dtype added to suppress error

    powerplants['box_id'] = idxbox(powerplants['latitude'], powerplants['longitude'])  # sort for box
    ...
    cols = ['gppd_idnr', 'name', 'capacity_mw', 'estimated_generation_gwh_2017', 'primary_fuel', 'box_id', 'geometry']
    ...
    for box_id, powerplants_box in tqdm(powerplants.groupby('box_id'), desc='saving powerplant csv', total=len(powerplants['box_id'].unique())):
    ...
    for id in tqdm(range(int(float(tot_boxes))), desc='empty folders', total=float(tot_boxes)):  # create empty ones
        all_boxes_pp_file = os.path.join("data","processed", "all_boxes", f'box_{id}', f'powerplants_box_{id}.csv')
