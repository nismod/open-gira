...
def required_nh_remaining_dict(rsn):
    """Checks which storms (unique identifier number_hur, abbreviated: nh) needs to be evaluated. Note that each of
       the lists' element locations correspond to the same event.
    Input:
        rsn: list [list of regions, list of samples, list of nh]
    Output:
        d_new: dict {region: sample: nh_lst...} <- updated only
    """
    region_all = rsn[0]
    sample_all = rsn[1]
    nh_all = rsn[2]
    ...
    d = dict()
    for r, s, nh in zip(region_remaining, sample_remaining, nh_remaining):
        if r not in d.keys():
            d[r] = dict()
        #if s not in d[r].keys()
        d[r][s] = nh
...
def find_nh_mult_dict(years_inp, regions_inp, samples_inp):
    """Given multiple regions and multiple samples, find the nh identifiers over the years years_inp
    Input:
        years_inp: list of years
        regions_inp: list of regions
    ...
    for region in regions_inp:
        if region not in d.keys():
            d[region] = dict()
        for sample in samples_inp:
            if sample not in d[region].keys():
                d[region][sample] = dict()
            add_nh = find_nh(
                years_inp, region, sample
            )  # for each region and sample, find the unique nh identifiers

            d[region][sample] = add_nh
    return [region_indiv, sample_indiv, nh_indiv]
...
rsn_req_dict = required_nh_remaining_dict(
    find_nh_mult(YEARS, REGIONS, SAMPLES)
)  # Dict # TODO add
...
rule intersect_winds_indiv:
    ...
    params:
        ...
        #req_nh = required_nh_remaining(find_nh_mult(YEARS, region, sample))
    output:
        [
            os.path.join(
                "data",
            ...
            for nh in rsn_req_dict["{region}"]["{sample}"]


            #"{params.nh_compute}" #rsn_req[2]  # TODO check
        ],
    script:
