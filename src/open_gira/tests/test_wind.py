...
class TestHollandWindModel:
    ...
    def test_1D(self):
        ...
        expected_result = np.array([0.0, 16.289285, 6.453159, 3.577267])
        np.testing.assert_allclose(
            holland_wind_model(*args), expected_result, rtol=1e-6
        )
...
class TestAdvectiveVector:
    ...
    def test_simple_model(self):
        ...
        np.testing.assert_allclose(advective_vector(0, 10, 1, 1, 0), 0 + 10j)

    def test_alpha_scaling(self):
        np.testing.assert_allclose(advective_vector(0, 10, 1, 0.5, 0), 0 + 5j)
    ...
    def test_beta_rotation_northern_hemisphere(self):
        ...
        np.testing.assert_allclose(advective_vector(0, 10, 1, 1, 90), -10 + 0j)
    ...
    def test_default_argument_regression(self):
        ...
        np.testing.assert_allclose(
            advective_vector(0, 10, 1), -1.84165322 + 5.28850767j
        )
...
class TestSigmoidDecay:
    ...
    def test_sigmoid_decay(self):
        ...
        np.testing.assert_allclose(
            expected, sigmoid_decay(np.arange(0, 1001, 50), 500, 0.004), rtol=1e-6
        )
...
class TestEstimateWindField:
    ...
    def test_estimate_wind_field_rotation_only(self):
        ...
        result = estimate_wind_field(
            np.linspace(-30.5, -29.5, 9), 15, -30, 15, 10000, 80, 92000, 100000, 0, 0
        )
        np.testing.assert_allclose(
            np.abs(result),
            np.array(
    ...
    def test_estimate_wind_field_advection_only(self):
        ...
        result = estimate_wind_field(
            np.linspace(-30.5, -29.5, 9), 15, -30, 15, 10000, 1e-6, 99999, 100000, 0, 10
        )
    ...
    def test_estimate_wind_field_rotation_and_advection(self):
        ...
        result = estimate_wind_field(
            np.linspace(-30.5, -29.5, 9), 15, -30, 15, 10000, 80, 90000, 100000, 0, 10
        )
